...
data_long <- read.table("FigS2e.sankey.csv", header=T, sep = ",")
nodes <- data.frame(name=c(as.character(data_long$Enriched), as.character(data_long$All)) %>% unique())#制作nodes
head(nodes)
data_long$IDsource=match(data_long$Enriched, nodes$name)-1 
data_long$IDtarget=match(data_long$All, nodes$name)-1
