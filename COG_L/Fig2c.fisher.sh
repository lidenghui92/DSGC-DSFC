...
#Input file1 DS_excl-cluster_members.list, contains the gene IDs of all COG/arCOG genes in excl-DS-clusters
#Input file2:
grep "^DSM_" COG_anno_by_PC.tsv |grep -v NA > DS_gene2uniqCOG.list #the COG_anno_by_PC.tsv can be generated with the script Fig2.COG_annotation_by_PC.sh
...
awk -F"\t" '{r=$3/$2;c='$1'-$3;d='$2'-'$1'-$4;print $1"\t"$2"\t"$3"\t"c"\t"$4"\t"d"\t"r}' DS_COG-L.stat2 > DS_COG-L.stat3
awk -F"\t" '{for(i=3;i<=6;i++){if($i==""){$i=0;}}print $0}' DS_COG-L.stat3> DS_COG-L.stat4
sed -i 's# #\t#g' DS_COG-L.stat4
mv DS_COG-L.stat4 DS_COG-L.stat  #this output file is provided as Fig2c.DS_COG-L.stat 

#run the test
Rscript Fig2c.fisher_test.R DS
rm DS_COG-L.stat2 DS_COG-L.stat3
awk '$NF<0.05' DS_COG-L.fisher.tsv > Fig2c.DS_COG-L.fisher
