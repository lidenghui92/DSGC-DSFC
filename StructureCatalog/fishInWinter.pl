...
=head1 Usage
  ...
  --fformat <str>     set fish file format, fasta|fasta_gz|table|gff|fq_gz
...
}elsif($FishFormat eq "fasta_gz" || (!$FishFormat && $fish_file=~/\.fa\.gz$/)){
	output_fasta_gz($fish_file,\%Bait,$FishColumn,$FishSeperator);
...
}elsif($FishFormat eq "fq_gz" || (!$FishFormat && $fish_file=~/\.fq\.gz$/)){    #add by gxy
	output_fq_gz($fish_file,\%Bait,$FishColumn,$FishSeperator);             #add by gxy
}else{
...
sub read_fasta_gz{
        my ($file,$bait_hp,$bait_colum,$bait_seperator) = @_;
        open(IN,"gzip -dc $file|")||die("fail to open $file\n");
        $/=">";<IN>;$/="\n";
        while (<IN>) {
                my $title=$_;
                chomp $title;
